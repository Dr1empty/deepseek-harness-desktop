...
Desktop 构建脚本校验两个插件的固定归档 SHA-256。皮肤从固定上游 Release 下载，Release 不可达时从固定 Git 提交生成同哈希归档；超级注入器因其上游源码构建依赖已经不可从 npm 获取，仓库保留一份未经修改的上游 v0.3.3 Release 归档。完整的 JavaScript 依赖许可清单可在打包后的 `LICENSES.chromium.html`、Harness 的 `THIRD_PARTY_NOTICES.md` 及各依赖包内查看。
