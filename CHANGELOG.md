...
- 在对话页模式选择器旁显示当前 DeepSeek 高峰/空闲计价时段、价格提示和下次切换倒计时。
- 按北京时间自动计算工作日计价切换，并在页面状态变化或计价时段变化后更新横幅。

## 1.1.7 - 2026-08-26

- 新增 Windows 托盘图标：关闭主窗口后应用继续在托盘运行，可从托盘重新打开、重启或彻底退出。
- 将本地用量统计移至后台线程，合并并发刷新请求，避免大量会话文件导致设置页面卡顿。
- 为用量与余额刷新增加超时保护、上次结果回退和后台刷新提示，慢速刷新时仍可继续操作界面。
